...
- 우선 채널: Android
- 다루는 장면: Android 패리티 로드맵을 통해 웹과 독립적으로 Android 채널을 설계하고 병렬 배포하는 구조

## 왜 중요한가
- Android 패리티 로드맵은 반복 사용 단계의 체감 시간을 줄이는 핵심 장면이다.
...
## 표준 흐름
1. 사용자는 Android 패리티 로드맵이 필요한 순간에 가장 가까운 표면에서 진입한다.
...
- 따라서 Android 패리티 로드맵은 단순 기능 제안이 아니라 현재 제품이 아직 불완전하게 느껴지는 지점을 메우는 문서로 본다.

## 측정과 릴리즈 게이트
- 첫 성공까지 걸리는 시간과 중간 이탈률을 함께 본다.
- 사용자가 Android 패리티 로드맵 흐름을 끝낸 뒤 다시 되돌아오지 않아도 되는 비율을 본다.
